...
export default async function handler(req, res) {
  ...
  try {
    ...
    const key = `videosLatest:${channelId}|${limit}|${slim}`;
    const cached = getFromMem(key);
    if (cached) { setCacheHeader(res, 300, 86400); return ok(res, cached); }

    // 1) Ambil upload terbaru (by date)
    ...
    setToMem(key, resp, 300);         // cache mem 5 menit
    setCacheHeader(res, 300, 86400);  // CDN cache
    ok(res, resp);
  } catch (e) {
    err(res, e);
  }
